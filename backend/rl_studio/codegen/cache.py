...
def _compute_hash(env_spec: Dict[str, Any], file_type: str, training_config: Optional[Dict[str, Any]], algorithm: str) -> str:
...
def get_cached_code(env_spec: Dict[str, Any], file_type: str, training_config: Optional[Dict[str, Any]], algorithm: str) -> Optional[Dict[str, Any]]:
    """
...
def set_cached_code(env_spec: Dict[str, Any], file_type: str, training_config: Optional[Dict[str, Any]], algorithm: str, code: str, file_name: str) -> None:
    ...
    logger.debug(f"Cached {file_type} (key: {cache_key[:8]}..., size: {len(code)} bytes)")
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    now = datetime.now()
    valid_entries = sum(1 for v in _code_cache.values() if now - v["timestamp"] < CACHE_TTL)
