...
def verify_asset_types(result: VerificationResult, client) -> bool:
    ...
    try:
        ...
        if missing:
            ...
            result.add_pass("Asset Types", f"Found {len(asset_types)} types: {', '.join(found_types)}")
...
def verify_assets(result: VerificationResult, client) -> bool:
    ...
    try:
        ...
        result.add_pass("Assets", f"Found {len(assets)} assets ({found_count}/{len(expected_assets)} expected)")
...
def verify_templates(result: VerificationResult, client) -> bool:
    ...
    try:
        ...
        if not templates or len(templates) == 0:
            result.add_warning("Templates", "No templates found - run seed script with project_id")
            return False
        ...
        result.add_pass("Templates", f"Found {len(templates)} templates ({found_count}/{len(expected_templates)} expected)")
