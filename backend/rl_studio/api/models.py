...
class Entity(BaseModel):
    ...
    components: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Component data")


class SceneGraph(BaseModel):
    """The scene graph containing all entities"""
    entities: List[Entity] = Field(default_factory=list)
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Scene metadata (gridConfig, tags, etc.)")


class ActionSpace(BaseModel):
    """Action space definition"""
    type: Literal["discrete", "continuous", "multi_discrete", "multi_binary", "dict"] = Field(..., description="Action space type")
    actions: Optional[List[str]] = Field(None, description="For discrete: list of action names")
    shape: Optional[Tuple[int, ...]] = Field(None, description="For continuous: shape of action vector")
    low: Optional[Tuple[float, ...]] = Field(None, description="For continuous: lower bounds")
    high: Optional[Tuple[float, ...]] = Field(None, description="For continuous: upper bounds")
    spaces: Optional[Dict[str, Any]] = Field(None, description="For dict: nested spaces")


class ObservationSpace(BaseModel):
    """Observation space definition"""
    type: Literal["box", "discrete", "multi_discrete", "multi_binary", "dict"] = Field(..., description="Observation space type")
    ...
    n: Optional[int] = Field(None, description="For discrete: number of discrete values")
    spaces: Optional[Dict[str, Any]] = Field(None, description="For dict: nested spaces")
...
class RLAgent(BaseModel):
    ...
    role: Literal["learning_agent", "scripted", "human"] = Field(default="learning_agent")
...
class RewardTrigger(BaseModel):
    ...
    type: Literal["enter_region", "exit_region", "collision", "event", "step", "custom"] = Field(..., description="Trigger type")
    entityId: Optional[str] = Field(None, description="Entity ID for entity-specific triggers")
    regionId: Optional[str] = Field(None, description="Region/entity ID for region triggers")
    eventName: Optional[str] = Field(None, description="Event name for event triggers")
    condition: Optional[Dict[str, Any]] = Field(None, description="Custom condition data")
...
class TerminationCondition(BaseModel):
    ...
    type: Literal["enter_region", "exit_region", "collision", "event", "max_steps", "custom"] = Field(..., description="Termination type")
...
class SpawnPoint(BaseModel):
    ...
    rotation: Optional[Tuple[float, float, float]] = Field(None, description="Spawn rotation")
...
class EpisodeConfig(BaseModel):
    ...
    reset: Dict[str, Any] = Field(..., description="Reset configuration (type, spawns, etc.)")
...
class CreateSceneRequest(BaseModel):
    ...
    projectId: Optional[str] = Field(None, description="Project/environment ID (optional)")
...
class CreateAssetRequest(BaseModel):
    ...
    assetTypeKey: str = Field(..., description="Asset type key: 'character', 'vehicle', 'prop', 'tile', 'prefab'")
    name: str = Field(..., description="Asset name")
    slug: Optional[str] = None
    thumbnailUrl: Optional[str] = None
    modelUrl: Optional[str] = None
    geometry: Optional[Dict[str, Any]] = Field(None, description="Procedural geometry: { primitive: 'rectangle'|'box'|'sphere'|'cylinder'|'curve', params: {...} }")
