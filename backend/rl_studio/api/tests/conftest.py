...
class MockConvexClient:
    ...
    def query(self, path: str, args: Dict[str, Any] = None) -> Any:
        ...
        if path == 'assetTypes/list':
            ...
            if args.get('mode'):
                assets = [a for a in assets if a.get('meta', {}).get('mode') == args['mode']]
            if args.get('tag'):
                tag = args['tag']
            ...
            key = args.get('key')
            return next((at for at in self.data['assetTypes'] if at.get('key') == key), None)
        
        elif path == 'assets/get':
            asset_id = args.get('id')
            return next((a for a in self.data['assets'] if a.get('_id') == asset_id), None)
        
        elif path == 'scenes/get':
            scene_id = args.get('id')
            scene = next((s for s in self.data['scenes'] if s.get('_id') == scene_id), None)
            if scene and scene.get('activeVersionId'):
                version = next(
                    (v for v in self.data['sceneVersions'] if v.get('_id') == scene['activeVersionId']),
                    None
                )
            ...
            return next((v for v in self.data['sceneVersions'] if v.get('_id') == version_id), None)
        
        elif path == 'templates/list':
            templates = self.data['templates']
            if args.get('isPublic') is not None:
                templates = [t for t in templates if t.get('isPublic') == args['isPublic']]
            return templates
        
        elif path == 'templates/get':
            template_id = args.get('id')
            template = next((t for t in self.data['templates'] if t.get('_id') == template_id), None)
            if template:
                version = next(
                    (v for v in self.data['sceneVersions'] if v.get('_id') == template.get('sceneVersionId')),
                    None
                )
                return {
                    'template': template,
            ...
            version = next(
                (v for v in self.data['sceneVersions'] 
                 if v.get('sceneId') == scene_id and v.get('versionNumber') == version_number),
                None
            )
            return version
    ...
    def mutation(self, path: str, args: Dict[str, Any] = None) -> Any:
        ...
        if path == 'scenes/create':
            ...
            scene_id = args.get('sceneId')
            existing_versions = [v for v in self.data['sceneVersions'] if v.get('sceneId') == scene_id]
            ...
            self.data['sceneVersions'].append(version)
            # Update scene's active version
            scene = next((s for s in self.data['scenes'] if s.get('_id') == version['sceneId']), None)
            if scene:
                scene['activeVersionId'] = version['_id']
            return version['_id']
            ...
            if original_version:
                ...
                version = {
                    ...
                    'sceneId': scene_id,
                    'versionNumber': 1,
                    'sceneGraph': json.loads(json.dumps(original_version.get('sceneGraph', {}))),  # Deep copy
                    'rlConfig': json.loads(json.dumps(original_version.get('rlConfig', {}))),  # Deep copy
                    'createdBy': args.get('createdBy'),
                    'createdAt': 1234567890,
                }
                self.data['sceneVersions'].append(version)
            ...
            asset_id = args.get('id')
            asset = next((a for a in self.data['assets'] if a.get('_id') == asset_id), None)
            if not asset:
            ...
            for key, value in args.items():
                if key != 'id':
                    if value is None:
                        ...
                        asset[key] = None
                    elif key in ['visualProfile', 'physicsProfile', 'behaviorProfile', 'meta', 'geometry']:
            ...
            if not asset:
                raise ValueError('Asset not found')
            self.data['assets'] = [a for a in self.data['assets'] if a.get('_id') != asset_id]
            return {'success': True}
...
def mock_convex_client(monkeypatch):
    ...
    from rl_studio.api import scenes, assets, templates, compile as compile_module
