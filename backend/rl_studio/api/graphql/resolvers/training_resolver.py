...
from ..types.training import TrainingRun, TrainingRunInput, JobStatus
from ....training.orchestrator import launch_training_job, get_job_status, stop_job
...
class TrainingResolver:
    ...
    async def launch_training(self, input: TrainingRunInput) -> TrainingRun:
        ...
        try:
            import json
            env_spec = json.loads(input.env_spec) if isinstance(input.env_spec, str) else input.env_spec
            config = json.loads(input.config) if isinstance(input.config, str) else input.config
