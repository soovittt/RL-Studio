...
from ..types.asset import Asset, AssetFilter, AssetInput, UpdateAssetInput
...
class AssetResolver:
        ...
        try:
            ...
            if filter and filter.search:
                ...
                assets = [
                    ...
                    or search_lower in a.asset_type.lower()
                ]
    ...
    async def create_asset(self, input: AssetInput) -> Asset:
        ...
        try:
            ...
            visual_profile = (
                json.loads(input.visual_profile) if input.visual_profile else {}
            )
            physics_profile = (
                json.loads(input.physics_profile) if input.physics_profile else {}
            )
            behavior_profile = (
                json.loads(input.behavior_profile) if input.behavior_profile else {}
            )
    ...
    async def update_asset(self, id: str, input: UpdateAssetInput) -> Asset:
        ...
        try:
            ...
            geometry = json.loads(input.geometry) if input.geometry else None
            visual_profile = (
                json.loads(input.visual_profile) if input.visual_profile else None
            )
            physics_profile = (
                json.loads(input.physics_profile) if input.physics_profile else None
            )
            behavior_profile = (
                json.loads(input.behavior_profile) if input.behavior_profile else None
            )
