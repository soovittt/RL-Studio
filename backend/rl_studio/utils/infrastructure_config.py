...
class InfrastructureConfig:
    ...
    def get_storage_config(self) -> Dict[str, Any]:
        ...
        if self.storage_provider == "s3":
            return {
                ...
                or os.getenv("MODEL_BUCKET_NAME"),
    ...
    def validate_storage_config(self) -> tuple[bool, Optional[str]]:
        ...
        if config["provider"] == "s3":
            ...
            if not config.get("connection_string") and not (
                config.get("account_name") and config.get("account_key")
            ):
                return (
                    False,
                    "AZURE_STORAGE_CONNECTION_STRING or (AZURE_STORAGE_ACCOUNT_NAME + AZURE_STORAGE_ACCOUNT_KEY) must be set",
                )
