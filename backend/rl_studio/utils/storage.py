...
def get_storage_client():
    ...
    if provider == "s3":
        try:
            ...
            logger.info(f"✅ Initialized S3 client (bucket: {storage_config.get('bucket_name')})")
            return client, "s3"
        except ImportError:
        ...
        try:
            ...
            logger.info(f"✅ Initialized GCS client (bucket: {storage_config.get('bucket_name')})")
            return client, "gcs"
        except ImportError:
        ...
        try:
            ...
            logger.info(f"✅ Initialized Azure Blob client (container: {storage_config.get('container_name')})")
            return client, "azure"
        except ImportError:
