...
class ConvexLocalDev:
    ...
    def __init__(self, convex_url: Optional[str] = None):
        self.convex_url = (
            convex_url or os.getenv("CONVEX_URL") or os.getenv("VITE_CONVEX_URL")
        )
    ...
    def seed_local_data(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"path": "seed_local_data:seedAll", "args": {}},
            ...
            if response.ok:
                ...
                if result.get("success"):
                    ...
                    logger.info(
                        f"   Environments: {result.get('results', {}).get('environments', {}).get('count', 0)}"
                    )
                    logger.info(
                        f"   Runs: {result.get('results', {}).get('runs', {}).get('count', 0)}"
                    )
                    logger.info(
                        f"   Evaluations: {result.get('results', {}).get('evaluations', {}).get('count', 0)}"
                    )
                    return True
                else:
    ...
    def get_environments(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = requests.post(
                f"{self.convex_http}/api/action",
                json={"path": "environments:list", "args": {}},
                headers={"Content-Type": "application/json"},
                timeout=10,
            )
    ...
    def get_runs(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            response = requests.post(
                f"{self.convex_http}/api/action",
                json={"path": "runs:listRecent", "args": {}},
                headers={"Content-Type": "application/json"},
                timeout=10,
            )
