...
from ..rollout.simulator import validate_env_spec, create_initial_state, select_action, step_simulator
...
class RewardRuleVerifier:
    ...
    def _check_unreachable_rewards(self, env_spec: Dict[str, Any], reward_rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Check for rewards that are likely unreachable"""
        issues = []
        
        # Check goal rewards - verify goals are reachable
        goal_rules = [r for r in reward_rules if r.get("condition", {}).get("type") == "goalReached"]
        ...
        if goals:
            ...
            if not reached_goal:
                issues.append({
                    "type": "unreachable_reward",
                    "rule_id": goal_rules[0].get("id", "unknown") if goal_rules else "unknown",
                    "message": "Goals may be unreachable (greedy policy couldn't reach them in 100 steps)",
                })
        
        return issues
    
    def _check_never_fire_rules(self, env_spec: Dict[str, Any], reward_rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _check_contradictory_rules(self, reward_rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _check_reward_hacking_risks(self, reward_rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Check for reward hacking risks"""
        warnings = []
        
        # Check for per-step rewards that are too high
        per_step_rules = [r for r in reward_rules if r.get("condition", {}).get("type") == "perStep"]
        ...
        stay_rules = [r for r in reward_rules if "stay" in str(r.get("condition", {})).lower()]
        if stay_rules:
            warnings.append({
                "type": "reward_hacking_risk",
    ...
    def _check_dense_shaping(self, reward_rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        shaping_rules = [r for r in reward_rules if r.get("condition", {}).get("type") not in terminal_types]
