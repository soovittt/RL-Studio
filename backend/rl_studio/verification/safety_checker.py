...
class SafetyChecker:
    ...
    def check(
        self, env_spec: Dict[str, Any], state: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    def _check_unsafe_states(
        self, state: Dict[str, Any], env_spec: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for agent in agents:
            ...
            if world.get("coordinateSystem") == "grid":
                if (
                    agent_pos.x < 0
                    or agent_pos.x >= world.get("width", 10)
                    or agent_pos.y < 0
                    or agent_pos.y >= world.get("height", 10)
                ):
                    issues.append(
                        {
                            "type": "unsafe_state",
