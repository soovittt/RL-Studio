...
def detect_algorithm_from_model(model_path: str) -> str:
    ...
    if (model_path_obj / "policy.pth").exists() or (model_path_obj / "policy.zip").exists():
    ...
    try:
        ...
        while not done and state["step"] < max_steps:
            ...
            step = {
                ...
                "action": action_str,
                "reward": state.get("totalReward", 0) - (steps[-1]["state"]["totalReward"] if steps else 0),
                "done": state.get("done", False),
            }
            steps.append(step)
        ...
        return {
            ...
            "terminationReason": "goal_reached" if success else "max_steps" if state["step"] >= max_steps else "unknown",
        }
