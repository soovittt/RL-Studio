...
def evaluate_condition(condition: Dict[str, Any], state: Dict[str, Any], env_spec: Dict[str, Any]) -> bool:
...
def calculate_reward(state: Dict[str, Any], env_spec: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def check_termination(state: Dict[str, Any], env_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if state.get("agents") and state.get("objects"):
        ...
        if isinstance(agent_pos_list, list) and len(agent_pos_list) >= 2:
            ...
            goals = [obj for obj in env_spec.get("objects", []) 
...
def apply_action(state: Dict[str, Any], action: Any, env_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(action, str):
        ...
        if world["coordinateSystem"] == "grid":
            ...
            if world["coordinateSystem"] == "cartesian":
                ...
                new_pos.y = max(-world["height"]/2, min(world["height"]/2, new_pos.y))
            else:
        ...
        if not hit_obstacle:
            ...
            new_state["info"]["events"].append(f"Moved {action} to ({new_pos.x:.1f}, {new_pos.y:.1f})")
        else:
            new_state["info"]["events"].append(f"Hit obstacle, stayed at ({agent_pos.x:.1f}, {agent_pos.y:.1f})")
...
def apply_action_to_agent(agent: Dict[str, Any], action: Any, state: Dict[str, Any], env_spec: Dict[str, Any], world: Dict[str, Any]) -> None:
    ...
    if isinstance(action, str):
        ...
        if not hit_obstacle:
            agent["position"] = new_pos.to_list()
            state["info"]["events"].append(f"Agent {agent_id} moved {action} to ({new_pos.x:.1f}, {new_pos.y:.1f})")
        else:
            state["info"]["events"].append(f"Agent {agent_id} hit obstacle, stayed at ({agent_pos.x:.1f}, {agent_pos.y:.1f})")
    elif isinstance(action, list) and len(action) >= 2:
        ...
        agent["position"] = [new_x, new_y]
        state["info"]["events"].append(f"Agent {agent_id} moved to ({new_x:.1f}, {new_y:.1f})")

def select_action(state: Dict[str, Any], env_spec: Dict[str, Any], policy: Literal["random", "greedy"]) -> Any:
    ...
    if action_space.get("type") == "discrete":
        ...
        if len(state.get("agents", [])) > 1:
            ...
            for agent in state["agents"]:
                ...
                if policy == "random":
                    ...
                    agent_actions[agent_id] = select_greedy_action_for_agent(agent, state, env_spec, actions)
        ...
        if policy == "random":
            ...
            goals = [obj for obj in env_spec.get("objects", []) 
                    if obj and isinstance(obj, dict) and obj.get("type") == "goal" 
                    and isinstance(obj.get("position"), list) and len(obj.get("position", [])) >= 2]
            if not goals:
                return random.choice(actions)
            ...
            def would_hit_obstacle(new_pos: Vec2) -> bool:
                # Check bounds first
                if world["coordinateSystem"] == "grid":
                    if new_pos.x < 0 or new_pos.x >= world["width"] or new_pos.y < 0 or new_pos.y >= world["height"]:
                        return True
                # Check obstacles
                for obj in state.get("objects", []):
                    if obj.get("type") in ["wall", "obstacle"]:
                        ...
                        if isinstance(obj_pos_list, list) and len(obj_pos_list) >= 2:
                            ...
                            if world["coordinateSystem"] == "grid":
                                if abs(new_pos.x - obj_pos.x) < 0.5 and abs(new_pos.y - obj_pos.y) < 0.5:
                                    return True
                            else:
                                if new_pos.distance(obj_pos) < 1.0:
                                    return True
            ...
            cell_size = world.get("cellSize", 1.0) if world["coordinateSystem"] == "grid" else 0.1
        ...
        if len(state.get("agents", [])) > 1:
            ...
            for agent in state["agents"]:
                agent_id = agent.get("id", "unknown")
                if policy == "random":
                    agent_actions[agent_id] = [random.uniform(-1, 1), random.uniform(-1, 1)]
                else:
                    agent_actions[agent_id] = select_greedy_continuous_action_for_agent(agent, state, env_spec)
            return agent_actions
...
def select_greedy_action_for_agent(agent: Dict[str, Any], state: Dict[str, Any], env_spec: Dict[str, Any], actions: List[str]) -> str:
    ...
    preferred_action = None
    if abs(dx) >= abs(dy):
        preferred_action = "right" if dx > 0 else "left" if abs(dx) >= 0.1 else ("up" if dy > 0 else "down")
    else:
        if world["coordinateSystem"] == "grid":
            preferred_action = "up" if dy > 0 else "down"
        else:
...
def select_greedy_continuous_action_for_agent(agent: Dict[str, Any], state: Dict[str, Any], env_spec: Dict[str, Any]) -> List[float]:
...
def create_initial_state(env_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "agents": [
            {
                "id": agent.get("id", ""),
                "position": (agent.get("position", [0, 0])[:] if isinstance(agent.get("position"), list) and len(agent.get("position", [])) >= 2 else [0, 0]),
        ...
        "objects": [
            {
                "id": obj.get("id", ""),
                "position": (obj.get("position", [0, 0])[:] if isinstance(obj.get("position"), list) and len(obj.get("position", [])) >= 2 else [0, 0]),
...
def step_simulator(state: Dict[str, Any], action: Any, env_spec: Dict[str, Any], max_steps: int) -> Dict[str, Any]:
    ...
    if termination["terminated"]:
        new_state["done"] = True
        new_state["info"]["events"].append(f"Terminated: {termination.get('reason', 'unknown')}")
...
def validate_env_spec(env_spec: Dict[str, Any]) -> tuple[bool, str]:
    ...
    for i, agent in enumerate(agents):
        ...
        try:
            ...
            if x < 0 or x >= world.get("width", 0) or y < 0 or y >= world.get("height", 0):
        ...
        if "position" in obj:
            ...
            if isinstance(pos, list) and len(pos) >= 2:
                try:
                    ...
                    if x < -world.get("width", 0) or x > world.get("width", 0) * 2 or \
                       y < -world.get("height", 0) or y > world.get("height", 0) * 2:
                        return False, f"Object {i} position ({x}, {y}) is way out of bounds"
                except (ValueError, TypeError):
    ...
    if not isinstance(reward_rules, list) or len(reward_rules) == 0:
        return False, "No reward rules defined. Please add reward rules in the Rules panel (right sidebar → Rewards tab)"
    ...
    if not isinstance(termination_rules, list) or len(termination_rules) == 0:
        return False, "No termination rules defined. Please add termination rules in the Rules panel (right sidebar → Terminations tab)"
...
def run_rollout(
    ...
    if not success:
        success = any(
            "goal" in event.lower() or "Goal" in event or "reached goal" in event.lower()
    ...
    return {
        ...
        "success": success,
        "terminationReason": state["info"]["events"][-1] if state["info"]["events"] else None
    }
