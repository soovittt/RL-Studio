...
class SensorModel:
    ...
    def sense(self, agent_pos: Vec2, agent_angle: float, state: Dict[str, Any], env_spec: Dict[str, Any]) -> np.ndarray:
...
class RaycastSensor(SensorModel):
    ...
    def sense(self, agent_pos: Vec2, agent_angle: float, state: Dict[str, Any], env_spec: Dict[str, Any]) -> np.ndarray:
        ...
        for ray_angle in ray_angles:
            ...
            # Cast ray
            distance = self._cast_ray(agent_pos, abs_angle, max_range, state, env_spec, world)
    ...
    def _cast_ray(
        ...
        for i in range(num_samples):
            ...
            if world.get("coordinateSystem") == "grid":
                if x < 0 or x >= world.get("width", 10) or y < 0 or y >= world.get("height", 10):
                    return t
            else:
                if world.get("coordinateSystem") == "cartesian":
                    if abs(x) > world.get("width", 10) / 2 or abs(y) > world.get("height", 10) / 2:
                        return t
...
class LIDARSensor(SensorModel):
    ...
    def sense(self, agent_pos: Vec2, agent_angle: float, state: Dict[str, Any], env_spec: Dict[str, Any]) -> np.ndarray:
        ...
        for i in range(num_beams):
            ...
            distance = self._cast_ray(agent_pos, angle, max_range, state, env_spec, world)
            distances.append(distance)
...
class SensorCalculator:
    ...
    @staticmethod
    def calculate_sensors(agent: Dict[str, Any], state: Dict[str, Any], env_spec: Dict[str, Any]) -> Dict[str, np.ndarray]:
