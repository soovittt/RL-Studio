...
class TerminationAnalyzer:
    ...
    def analyze_rollout(self, rollout_steps: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if done:
            termination_reason = state.get("info", {}).get("termination_reason", "unknown")
    ...
    def analyze_multiple_rollouts(self, rollouts: List[List[Dict[str, Any]]]) -> Dict[str, Any]:
            ...
            for reason, count in termination_counts.items():
                if count / total_terminations > 0.3 and len(termination_counts) > 1:
                    conflicting_rules.append({
                        ...
                        "conflict_with": [r for r in termination_counts.keys() if r != reason],
                    })
        ...
        for reason, steps in termination_step_distributions.items():
            if steps:
                ...
                heatmap_data.append({
                    ...
                    "count": len(steps),
                    "skewness": float(stats.skew(steps_array)),  # Distribution shape
                    "kurtosis": float(stats.kurtosis(steps_array)),  # Tail heaviness
                })
        ...
        return {
            ...
            "premature_terminations": self._detect_premature_terminations(termination_step_distributions),
            "late_terminations": self._detect_late_terminations(termination_step_distributions),
        }
    
    def _detect_premature_terminations(self, distributions: Dict[str, List[int]], threshold_percentile: int = 10) -> List[Dict[str, Any]]:
    ...
    def _detect_late_terminations(self, distributions: Dict[str, List[int]], threshold_percentile: int = 90) -> List[Dict[str, Any]]:
