...
class TrajectoryAnalyzer:
    ...
    def analyze_rollout(
        self, rollout_steps: List[Dict[str, Any]], env_spec: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if total_actions > 0:
            probs = np.array(
                [count / total_actions for count in action_distribution.values()]
            )
    ...
    def analyze_multiple_rollouts(
        self, rollouts: List[List[Dict[str, Any]]], env_spec: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _detect_oscillations(
        self, trajectory: List[Dict[str, Any]], window: int = 5
    ) -> Dict[str, Any]:
        ...
        if len(distances) > window:
            ...
            oscillation_score = (
                np.mean(autocorr[:window]) if len(autocorr) >= window else 0
            )
    ...
    def _detect_attractors(
        self, trajectory: List[Dict[str, Any]], radius: float = 1.0
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_entropy_over_time(
        self, trajectories: List[List[Dict[str, Any]]]
    ) -> List[float]:
    ...
    def _find_common_paths(
        self, trajectories: List[List[Dict[str, Any]]], min_length: int = 3
    ) -> List[Dict[str, Any]]:
