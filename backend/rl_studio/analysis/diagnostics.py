...
class RLDiagnostics:
    ...
    def compute_value_function_heatmap(
        ...
        return {
            ...
            "min_value": float(np.min(heatmap[counts > 0])) if np.any(counts > 0) else 0,
            "max_value": float(np.max(heatmap[counts > 0])) if np.any(counts > 0) else 0,
            "grid_size": grid_size,
        }
    ...
    def get_diagnostics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "policy_entropy": {
                "mean": np.mean(self.policy_entropy_history) if self.policy_entropy_history else 0,
                "std": np.std(self.policy_entropy_history) if self.policy_entropy_history else 0,
                "trend": "increasing" if len(self.policy_entropy_history) > 1 and 
                        self.policy_entropy_history[-1] > self.policy_entropy_history[0] else "decreasing",
            },
            "kl_divergence": {
