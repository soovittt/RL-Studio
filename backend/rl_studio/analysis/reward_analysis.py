        ...
        for rule_id, contributions in self.rule_contributions.items():
            if len(contributions) > 0:
                analysis["per_rule_stats"][rule_id] = {
                    ...
                    "fire_rate": self.rule_fire_counts[rule_id] / len(self.step_rewards) if self.step_rewards else 0,
                }
        
        # Most active rules (by fire count)
        analysis["most_active_rules"] = sorted(
            self.rule_fire_counts.items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]
...
class RewardAnalyzer:
    ...
    def analyze_rollout(self, rollout_steps: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis["reward_density"] = float(np.mean(rewards_array)) if len(rewards_array) > 0 else 0.0
    ...
    def analyze_multiple_rollouts(self, rollouts: List[List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    def _detect_issues(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        for rule_id, stats in analysis["per_rule_stats"].items():
            if stats["fire_count"] == 0:
                ...
                warnings.append(f"Rule {rule_id} fires very rarely ({stats['fire_rate']:.1%})")
        ...
        rule_values = {rule_id: stats["mean"] for rule_id, stats in analysis["per_rule_stats"].items()}
    ...
    def _analyze_rule_consistency(self, analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for rule_id, rates in rule_fire_rates.items():
            consistency[rule_id] = {
                ...
                "consistency": "high" if np.std(rates) < 0.1 else "medium" if np.std(rates) < 0.3 else "low",
            }
        
        return consistency
    
    def _get_top_termination_causes(self, rollouts: List[List[Dict[str, Any]]]) -> List[Tuple[str, int]]:
        ...
        for rollout in rollouts:
            if rollout:
                last_step = rollout[-1]
                termination_reason = last_step.get("state", {}).get("info", {}).get("termination_reason", "unknown")
