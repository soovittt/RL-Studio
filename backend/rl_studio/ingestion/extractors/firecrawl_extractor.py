...
class FirecrawlExtractor(BaseExtractor):
    ...
    async def extract(self, input_data: str, **kwargs) -> ExtractionResult:
        ...
        try:
            ...
            response = await self._scrape_url(input_data, firecrawl_api_key, scrape_options)
            ...
            if not response.get("success"):
                return ExtractionResult(
                    ...
                    normalized_data={},
                    metadata=self._create_metadata(
                        input_data, "firecrawl_scrape", 0.0,
            ...
            return ExtractionResult(
                ...
                normalized_data={},
                metadata=self._create_metadata(
                    input_data, "firecrawl_error", 0.0,
                    [f"Exception: {str(e)}"]
                ),
                error=str(e)
            )
    
    async def _scrape_url(self, url: str, api_key: str, options: Dict[str, Any]) -> Dict[str, Any]:
        """Call Firecrawl API"""
        import aiohttp
    ...
    def _detect_env_type(self, lower_content: str, content: str) -> str:
        ...
        if any(term in lower_content for term in ["continuous", "physics", "velocity", "acceleration", "force"]):
            return "continuous2d"
        elif any(term in lower_content for term in ["grid", "discrete", "cell", "tile"]):
            return "grid"
        elif any(term in lower_content for term in ["graph", "node", "edge"]):
            return "graph"
        else:
            # Default based on action space mentions
            if "continuous action" in lower_content or "continuous control" in lower_content:
                return "continuous2d"
            return "grid"
    ...
    def _extract_actions(self, content: str, lower_content: str, env_type: str) -> Dict[str, Any]:
    ...
    def _extract_rewards(self, content: str, lower_content: str) -> List[Dict[str, Any]]:
    ...
    def _extract_terminations(self, content: str, lower_content: str) -> List[Dict[str, Any]]:
    ...
    def _extract_objects(self, content: str, lower_content: str) -> List[Dict[str, Any]]:
    ...
    def _extract_physics(self, content: str, lower_content: str) -> Dict[str, Any]:
        ...
        # Extract gravity
        gravity_match = re.search(r'gravity[:\s=]+[\[\(]?([+-]?\d*\.?\d+)[,\s]+([+-]?\d*\.?\d+)[\]\)]?', content, re.IGNORECASE)
        if gravity_match:
            physics["gravity"] = [float(gravity_match.group(1)), float(gravity_match.group(2))]
        
        # Extract friction
        friction_match = re.search(r'friction[:\s=]+([+-]?\d*\.?\d+)', content, re.IGNORECASE)
        if friction_match:
            physics["friction"] = float(friction_match.group(1))
        
        return physics
    
    def _build_normalized(self, parsed: Dict[str, Any], scrape_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": scrape_data.get("markdown", "")[:500] if isinstance(scrape_data.get("markdown"), str) else "",
            "world": {
    ...
    def _calculate_confidence(self, parsed: Dict[str, Any], normalized: Dict[str, Any]) -> float:
    ...
    def _collect_warnings(self, parsed: Dict[str, Any], normalized: Dict[str, Any]) -> List[str]:
