...
class EnvSpecBuilder:
    ...
    def build(self, unification_result: UnificationResult) -> BuildResult:
        ...
        try:
            ...
            # Collect warnings
            warnings = unification_result.warnings + self._collect_build_warnings(env_spec)
    ...
    def _build_world_spec(self, unified: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "coordinateSystem": world.get("coordinateSystem", "grid" if env_type == "grid" else "cartesian"),
    ...
    def _build_agent_specs(self, unified: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for agent in agents:
            enhanced = {
                ...
                "dynamics": agent.get("dynamics", {
                    "type": "grid-step" if env_type == "grid" else "continuous-velocity",
                    "maxSpeed": agent.get("maxSpeed", 1.0) if env_type != "grid" else None
                }),
    ...
    def _build_action_space(self, unified: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not action_space:
            if env_type == "grid":
                action_space = {"type": "discrete", "actions": ["up", "down", "left", "right"]}
            else:
    ...
    def _build_metadata(self, unified: Dict[str, Any], unification_result: UnificationResult) -> Dict[str, Any]:
        """Build metadata"""
        return {
    ...
    def _validate_env_spec(self, env_spec: Dict[str, Any]) -> List[str]:
        ...
        required_fields = ["id", "name", "envType", "world", "agents", "actionSpace", "rules"]
        ...
        if action_space.get("type") == "discrete":
            ...
            if not isinstance(action_space.get("range"), list) or len(action_space.get("range", [])) < 2:
