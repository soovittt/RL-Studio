...
class FirecrawlService {
  ...
  async crawl(url: string, options?: { maxDepth?: number; includeSubdomains?: boolean }): Promise<FirecrawlScrapeResponse> {
...
class EnvironmentParser {
  ...
  private extractWorldDimensions(content: string, envType: string): { width?: number; height?: number; coordinateSystem?: 'grid' | 'cartesian' } {
  ...
  private extractObjects(content: string, envType: string): Array<{ type: string; position?: [number, number]; properties?: Record<string, any> }> {
    const objects: Array<{ type: string; position?: [number, number]; properties?: Record<string, any> }> = []
    
    // Look for ASCII grid patterns (improved)
    const asciiGrid = content.match(/```[\s\S]*?```/g) || 
                     content.match(/(?:^|\n)[#\sSGOTKDL]+(?:\n[#\sSGOTKDL]+)+/gm)
    if (asciiGrid) {
      for (const grid of asciiGrid) {
        ...
        for (let y = 0; y < lines.length; y++) {
          ...
          for (let x = 0; x < line.length; x++) {
            ...
            } else if (char === 'S' || char === 's' || char === 'A' || char === 'a' || char === '🤖') {
              objects.push({ type: 'agent', position: [x, y + startY] })
            } else if (char === 'O' || char === 'o' || char === 'X' || char === 'x' || char === '■') {
  ...
  private extractAgents(content: string): Array<{ name?: string; position?: [number, number] }> {
    ...
    const agentMatches = Array.from(content.matchAll(/(?:agent|player|robot)[:\s]+(?:position|start|spawn)[:\s]+\(?(\d+)[,\s]+(\d+)\)?/gi))
  ...
  private extractActionSpace(content: string): { type: 'discrete' | 'continuous'; actions?: string[]; dimensions?: number } | undefined {
    const lowerContent = content.toLowerCase()
    
    // Discrete actions
    if (lowerContent.match(/\b(discrete|action space|up|down|left|right|north|south|east|west)\b/)) {
  ...
  extractRewards(content: string): Array<{ condition: string; value: number; description?: string }> {
    ...
    if ((lowerContent.includes('goal') || lowerContent.includes('target')) && 
        (lowerContent.includes('reward') || lowerContent.includes('+10') || lowerContent.includes('+1'))) {
    ...
    if (lowerContent.includes('step') && (lowerContent.includes('penalty') || lowerContent.includes('-0.1') || lowerContent.includes('-0.01'))) {
  ...
  private extractMetadata(content: string, sourceUrl?: string): { source?: string; tags?: string[]; notes?: string } {
    const tags: string[] = []
    const lowerContent = content.toLowerCase()
    
    // Extract tags based on content
    if (lowerContent.includes('grid') || lowerContent.includes('maze')) tags.push('gridworld')
    if (lowerContent.includes('continuous') || lowerContent.includes('physics')) tags.push('continuous')
...
export const scrapeUrl = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!firecrawlApiKey) {
      throw new Error('Firecrawl API key not configured. Please set FIRECRAWL_API_KEY in your Convex dashboard (Settings > Environment Variables) or run: npx convex env set FIRECRAWL_API_KEY your-key-here')
    }
...
export const suggestRewardRules = action({
  ...
  handler: async (ctx, args) => {
    ...
    const suggestions: Array<{ condition: string; value: number; description: string; source?: string }> = []
    ...
    if (args.envDescription.toLowerCase().includes('navigation') || 
...
export const extractCurriculumPatterns = action({
  ...
  handler: async (ctx, args) => {
    ...
    const patterns: Array<{ name: string; description: string; phases: Array<{ difficulty: string; settings: Record<string, any> }> }> = []
...
export const extractDomainRandomizationRecipes = action({
  ...
  handler: async (ctx) => {
    ...
    const recipes: Array<{ name: string; parameters: Array<{ name: string; range: [number, number] }>; description: string }> = []
...
export const parseDiagram = action({
  ...
  handler: async (ctx, args) => {
    ...
    const asciiMatches = content.match(/```[\s\S]*?```/g) || 
                        content.match(/(?:^|\n)[#\sSGOTKDL]+(?:\n[#\sSGOTKDL]+)+/gm)
...
## Environment Type
${envSpec.envType === 'grid' ? 'Grid-based discrete environment' : 
  envSpec.envType === 'continuous2d' ? '2D Continuous control environment' : 
...
${Array.from(objectTypes.entries()).map(([type, count]) => `- **${type}**: ${count}`).join('\n')}
...
Total objects: ${envSpec.objects.length}`,
    ...
    if (envSpec.agents && envSpec.agents.length > 0) {
      sections.push({
        title: 'Agents',
        content: `## Agents
${envSpec.agents.map((agent: { name?: string; position: [number, number] }, idx: number) => 
...
${envSpec.rules.rewards.map((reward: { condition: { type: string }; value: number; description?: string }, idx: number) => 
  `### Reward ${idx + 1}
...
${envSpec.rules.terminations.map((term: { condition: { type: string; steps?: number }; description?: string }, idx: number) => 
  `### Condition ${idx + 1}
