...
export function CreateRun({ envId, onClose }: CreateRunProps) {
  ...
  const [advancedHyperparams, setAdvancedHyperparams] = useState({
    clip_range: 0.2,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white border border-gray-200 rounded-lg shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">New Training Run</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-900">Algorithm</label>
            <select
              value={algorithm}
              onChange={(e) => setAlgorithm(e.target.value as AlgorithmType)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
            >
              {algorithmOptions
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-900">RL Concepts</label>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-3 pl-1">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Gamma</label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Total Steps</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Batch Size</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Rollout Length</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Update Epochs</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Entropy Coefficient</label>
                  <input
                    type="number"
                    step="0.001"
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-900">Training Configuration</label>
            <div className="space-y-3 pl-1">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">GPU Accelerator</label>
                  <select
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Rollout Interval (steps)</label>
                  <input
                    type="number"
                    min="10"
                    max="10000"
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  />
                  <p className="text-xs text-gray-500 mt-1">How often to send metrics</p>
                </div>
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Seed</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Parallel Environments</label>
                  <input
                    type="number"
                    min="1"
                    max="32"
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1.5 font-medium">Checkpoint Interval (steps)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer pt-6">
                    ...
                    />
                    <span className="text-xs text-gray-600 font-medium">Early Stopping</span>
                  </label>
                  {trainingConfig.early_stopping && (
                    <input
                      ...
                      className="w-full mt-2 px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  )}
                </div>
              </div>
          ...
          {envSpec && (
            <div className="space-y-2 bg-gray-50 p-4 rounded-md border border-gray-200">
              <label className="block text-sm font-semibold text-gray-900">Environment Summary (Read-only)</label>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-gray-600">Env Name:</span>
                  <span className="ml-2 text-gray-900 font-medium">{envSpec.name || 'Untitled'}</span>
                </div>
                <div>
                  <span className="text-gray-600">Type:</span>
                  <span className="ml-2 text-gray-900 font-medium capitalize">{envSpec.envType || 'grid'}</span>
                </div>
                <div>
                  <span className="text-gray-600">Agents:</span>
                  <span className="ml-2 text-gray-900 font-medium">{envSpec.agents?.length || 0} agent{envSpec.agents?.length !== 1 ? 's' : ''}</span>
                </div>
                <div>
                  <span className="text-gray-600">Action Space:</span>
                  <span className="ml-2 text-gray-900 font-medium">{getActionSpaceSummary()}</span>
                </div>
                <div>
                  <span className="text-gray-600">Observation Space:</span>
                  <span className="ml-2 text-gray-900 font-medium">{getObservationSpaceSummary()}</span>
                </div>
                <div>
                  <span className="text-gray-600">Reward Rules:</span>
                  <span className="ml-2 text-gray-900 font-medium">{envSpec.rules?.rewards?.length || 0}</span>
                </div>
                <div>
                  <span className="text-gray-600">Termination Rules:</span>
                  <span className="ml-2 text-gray-900 font-medium">{envSpec.rules?.terminations?.length || 0}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {envSpec && (
            <div className="space-y-2 bg-gray-50 p-4 rounded-md border border-gray-200">
              <label className="block text-sm font-semibold text-gray-900">Action & Observation Space (Read-only)</label>
              <div className="space-y-3 text-sm">
                <div>
                  <span className="text-gray-600 font-medium">Action Space:</span>
                  <div className="mt-1 text-gray-900 bg-white p-2 rounded border border-gray-200 font-mono text-xs">
                    {getActionSpaceSummary()}
                  </div>
                </div>
                <div>
                  <span className="text-gray-600 font-medium">Observation Space:</span>
                  <div className="mt-1 text-gray-900 bg-white p-2 rounded border border-gray-200 font-mono text-xs">
          ...
          <div className="space-y-2 border border-gray-200 rounded-md">
            <button
              type="button"
              onClick={() => setShowAdvanced(!showAdvanced)}
              className="w-full flex justify-between items-center p-3 text-sm font-semibold text-gray-900 hover:bg-gray-50 transition-colors"
            >
              ...
              <div className="p-4 space-y-4 border-t border-gray-200">
                ...
                {(algorithm === 'ppo' || algorithm === 'a2c') && (
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold text-gray-700 uppercase">PPO/A2C Settings</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Clip Range</label>
                        <input
                          type="number"
                          step="0.01"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Value Loss Coeff</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Max Grad Norm</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">GAE Lambda</label>
                        <input
                          type="number"
                          step="0.01"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Mini-batch Size</label>
                        <input
                          type="number"
                ...
                {algorithm === 'dqn' && (
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold text-gray-700 uppercase">DQN Settings</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Replay Buffer Size</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Target Update Freq</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Epsilon Start</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Epsilon End</label>
                        <input
                          type="number"
                          step="0.01"
                ...
                {concepts.imitation && (
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold text-gray-700 uppercase">Imitation Learning Settings</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Expert Dataset Path</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1.5">Mixture Coefficient</label>
                        <input
                          type="number"
                          step="0.1"
          ...
          {serviceError && (
            <div className="bg-red-50 border border-red-200 rounded-md p-3.5 text-sm text-red-800 flex items-start gap-2">
          ...
          {serviceAvailable === false && !serviceError && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3.5 text-sm text-yellow-800 flex items-start gap-2">
              <span className="text-lg leading-none">⚠️</span>
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-gray-200">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700"
            >
              Cancel
            </button>
            <button
              ...
              className="px-5 py-2.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm font-medium shadow-sm hover:shadow"
            >
              {isSubmitting ? (
