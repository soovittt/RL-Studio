...
export function TrainingVisualization({ 
  ...
  if (displaySpec.envType === 'grid') {
    ...
    const worldToGrid = (worldX: number, worldY: number) => [Math.floor(worldX / cellSize), Math.floor(worldY / cellSize)]

    const getObjectAt = (gridX: number, gridY: number) => {
      const worldPos = gridToWorld(gridX, gridY)
      return displaySpec.objects.find((obj) => {
    ...
    const getAgentAt = (gridX: number, gridY: number) => {
      const worldPos = gridToWorld(gridX, gridY)
      return displaySpec.agents.find((agent) => {
        const [agentX, agentY] = agent.position
        return Math.floor(agentX) === Math.floor(worldPos[0]) && 
            ...
            {Array.from({ length: height }).map((_, y) =>
              Array.from({ length: width }).map((_, x) => {
                ...
                return (
                  <div
                    key={`${x}-${y}`}
                    className={`w-10 h-10 border border-gray-300 ${color}`}
                  />
                )
              })
            )}
