...
export function assetToRender2D(asset: Asset, overrideColor?: string): Render2D {
  const defaultRender = getDefaultRender2D(`asset_${asset.name.toLowerCase().replace(/\s+/g, '_')}`, asset)
  
  return {
    shape: defaultRender.shape,
    size: defaultRender.size,
    radius: defaultRender.radius,
    color: overrideColor || asset.meta?.paletteColor || asset.visualProfile?.color || defaultRender.color,
 ...
 */
export function assetToAgent(asset: Asset, actionSpace: 'grid_moves_4' | 'grid_moves_8' | 'none' | 'custom' = 'grid_moves_4'): Agent | null {
 ...
 */
export function assetToTriggerZone(asset: Asset, triggerType: 'goal' | 'trap' | 'reward' | 'penalty'): TriggerZone | null {
  if (triggerType === 'goal') {
    return {
 ...
 */
export function getTriggerTypeFromAsset(asset: Asset): 'goal' | 'trap' | 'reward' | 'penalty' | null {
